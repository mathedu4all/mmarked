## [3.1.10](https://cloud.mathcrowd.cn:2444/agile/frontend/mathcrowd-marked-lib/compare/v3.1.9...v3.1.10) (2026-09-02)


### Bug Fixes

* **markdown:** add footnoteOffset parameter to renderMarkdown ([252af0f](https://cloud.mathcrowd.cn:2444/agile/frontend/mathcrowd-marked-lib/commits/252af0fcadda0a8e4f6dd7c1e8484c874aed9f8e)), closes [#22](https://cloud.mathcrowd.cn:2444/agile/frontend/mathcrowd-marked-lib/issues/22)



## [3.1.9](https://cloud.mathcrowd.cn:2444/agile/frontend/mathcrowd-marked-lib/compare/v3.1.8...v3.1.9) (2026-08-24)


### Bug Fixes

* **mathjax:** skip MathJax entirely when html has no TeX expressions ([d1f8875](https://cloud.mathcrowd.cn:2444/agile/frontend/mathcrowd-marked-lib/commits/d1f88752ec1b9bcabd608290f3f3bf74c54e77db))
...
* refactor(license)!: 公共 API 只保留配置与查询，移除可一行关掉授权执行的导出（issue #20） ([a7fd43c](https://cloud.mathcrowd.cn:2444/agile/frontend/mathcrowd-marked-lib/commits/a7fd43cf5447fb0e1be306171932d9ec1dab6258)), closes [#20](https://cloud.mathcrowd.cn:2444/agile/frontend/mathcrowd-marked-lib/issues/20) [#20](https://cloud.mathcrowd.cn:2444/agile/frontend/mathcrowd-marked-lib/issues/20)


### Bug Fixes

* restore missing demo/index.html ([02c9ef5](https://cloud.mathcrowd.cn:2444/agile/frontend/mathcrowd-marked-lib/commits/02c9ef502545e2ddab84da954b736f1f1a704ba9))


### BREAKING CHANGES

* 包根不再导出 setUnlicensedWarningInterval、
UNLICENSED_WARNING_INTERVAL、trackFunctionCall、getWarningIfNeeded、
resetUsageStats、stopUsageTracking、clearValidationCache。

`export * from './license'` 把内部机制整体抬到了公共 API 面上。检查公开发布的
@mathcrowd/mmarked@3.1.6 产物确认这些符号确实在导出列表里，于是：

    import { setUnlicensedWarningInterval } from '@mathcrowd/mmarked'
    setUnlicensedWarningInterval(Number.MAX_SAFE_INTEGER)   // 警告永不触发

一行即可关掉授权执行，stopUsageTracking() 同理关掉用量上报，**都不需要任何
key**。也就是说偷 key 反而是更麻烦、更容易留痕的路径，直接关掉才是理性选择。

保留的公共 API：configureLicense / getLicenseConfig / isLicensed /
validateLicense，以及 LicenseConfig / LicenseValidationResult / UsageStats
三个类型。后两个查询函数是只读的，不构成绕过入口。

被移除的函数注释本就写着 "for testing/debugging"，且 license.test.ts 一直是从
'./license' 内部导入，不依赖包根再导出——测试无需改动，15 例全绿。

影响面：已确认 mathcrowd-h5 / mathedu4all-site / omath.fun / mathcrowd-rst-lib /
landing-components 均未使用这些符号，下游升级无需改代码。

这只是抬高绕过成本、让内部机制不再是公开契约，并不能"防住"——服务端库的代码在
对方机器上跑，理论上防不住。让绕过留下痕迹的方案（许可证状态与渲染结果耦合）
